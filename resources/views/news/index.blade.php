                ...
                <div class="bg-white shadow-xl shadow-slate-200/60 rounded-3xl border border-slate-100">
                    <div class="p-6 lg:p-10">
                        ...
                        <div class="mb-8 rounded-2xl border border-slate-200 bg-slate-50/70 p-4 relative z-[100]">
                            <form id="filter-form" method="GET" action="{{ route('news.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6 items-end relative z-[100]">
                                <!-- Cari Judul -->
                                <div class="flex flex-col xl:col-span-2">
                                    <label for="filter_search" class="mb-1.5 block text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Cari Berita</label>
                                    <div class="relative group" x-data="{ value: '{{ $filters['search'] ?? '' }}' }">
                                        <input type="text" id="filter_search" name="search" x-model="value" placeholder="Cari judul berita..." class="w-full rounded-xl border border-slate-200 bg-white px-4 py-[7px] pr-10 text-[13px] font-bold focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 shadow-sm min-h-[38px]" :class="value ? 'text-slate-600' : 'text-slate-400'">
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-white overflow-visible">
                            <div class="overflow-visible">
                                <table class="min-w-full divide-y divide-slate-100">
                                    ...
                                    <tbody class="divide-y divide-slate-100 bg-white">@forelse($data as $key => $item)
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-16 text-center">
                                                    <div class="flex flex-col items-center justify-center">
                                                        ...
                                                        <h3 class="text-lg font-semibold text-slate-700">Tidak ada berita ditemukan</h3>
                                                        <p class="text-sm text-slate-500 max-w-xs mx-auto mt-1">Silakan tambahkan data berita baru.</p>
                                                        <a href="{{ route('news.create') }}" class="mt-4 inline-block text-sm text-blue-600 hover:text-blue-800 font-medium hover:underline">
                                                            + Tambah Berita
                                                        </a>
                                                    </div>
                                                </td>
