                    ...
                    <div class="p-6 lg:p-10">
                        ...
                        <div class="relative z-[250] flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-6">
                            <div>
                                ...
                                <p class="text-sm text-slate-500 mt-1">Kelola hasil penilaian wawancara seleksi beasiswa.</p>
                            </div>
                        ...
                        <div class="mb-8 rounded-2xl border border-slate-200 bg-slate-50/70 p-4 relative z-[100]">
                            <form method="GET" action="{{ route('interview-assessments.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6 items-end">
                                <div class="flex flex-col xl:col-span-2">
                                    <label for="filter_search" class="mb-1.5 block text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Cari Penilai / Mahasiswa</label>
                                    <div class="relative" x-data="{ value: '{{ $filters['search'] ?? '' }}' }">
                                        <input type="text" id="filter_search" name="search" x-model="value"
                                            placeholder="Nama penilai atau mahasiswa..."
                                ...
                                <table class="min-w-full divide-y divide-slate-100">
                                    <thead class="bg-slate-50/80">
                                        <tr>
                                            ...
                                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Penilai</th>
                                            <th class="px-6 py-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Jadwal Wawancara</th>
                                            <th class="px-6 py-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Nilai</th>
                                        ...
                                        @forelse($data as $key => $item)
                                            <tr class="group hover:bg-blue-50/40 transition-colors duration-200">
                                                ...
                                                <td class="px-6 py-4 text-left">
                                                    <div class="font-bold text-blue-600">{{ $item->interview->application->student->name }}</div>
                                                    <div class="text-xs text-slate-500">{{ $item->interview->application->scholarship->scholarship_name }}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-semibold text-slate-700">
                                                    {{ $item->interviewer }}
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <div class="text-sm font-bold text-slate-800">{{ $item->interview->schedule->translatedFormat('d M Y') }}</div>
                                                    <div class="text-xs text-slate-500">{{ $item->interview->schedule->format('H:i') }} WIB</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                            ...
                                            <tr>
                                                <td colspan="6" class="px-6 py-16 text-center">
                                                    <div class="flex flex-col items-center justify-center">
                                                        ...
                                                        <h3 class="text-lg font-semibold text-slate-700">Tidak ada penilaian ditemukan</h3>
                                                        <p class="text-sm text-slate-500 max-w-xs mx-auto mt-1">Belum ada data penilaian wawancara yang dimasukkan.</p>
                                                        <a href="{{ route('interview-assessments.create') }}" class="mt-4 inline-block text-sm text-blue-600 hover:text-blue-800 font-medium hover:underline">
                                                            + Tambah Penilaian
                                                        </a>
                                                    </div>
                                                </td>
