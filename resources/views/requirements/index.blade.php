<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <h2 class="font-extrabold text-2xl text-gray-800 leading-tight tracking-tight">Persyaratan</h2>
            <nav class="flex text-sm font-medium text-gray-500">
                <a href="{{ route('dashboard') }}" class="hover:text-blue-600 cursor-pointer transition">Dashboard</a>
                <span class="mx-2">/</span>
                <span class="text-blue-600">Persyaratan</span>
            </nav>
        </div>
    </x-slot>
                ...
                <div class="bg-white shadow-xl shadow-slate-200/60 rounded-3xl border border-slate-100">
                    <div class="p-6 lg:p-10">
                        ...
                        <div class="relative z-[250] flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-6">
                            <div>
                                <h3 class="text-xl font-bold text-slate-800">Daftar Persyaratan</h3>
                                <p class="text-sm text-slate-500 mt-1">Kelola persyaratan yang dibutuhkan untuk pendaftaran beasiswa.</p>
                            </div>
                        ...
                        <div class="mb-8 rounded-2xl border border-slate-200 bg-slate-50/70 p-4 relative z-[100]">
                            <form method="GET" action="{{ route('requirements.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6 items-end">
                                <div class="flex flex-col xl:col-span-2">
                                    <label for="filter_search" class="mb-1.5 block text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Cari Persyaratan</label>
                                ...
                                <table class="min-w-full divide-y divide-slate-100">
                                    <thead class="bg-slate-50/80">
                                        <tr>
                                            ...
                                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Persyaratan</th>
                                            <th class="px-6 py-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Beasiswa Terkait</th>
                                        ...
                                        @forelse($data as $key => $item)
                                            <tr class="group hover:bg-blue-50/40 transition-colors duration-200">
                                                ...
                                                <td class="px-6 py-4 text-left">
                                                    <div class="font-bold text-blue-600">{{ $item->requirement_name }}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold
                                                        {{ $item->scholarshipRequirements->count() > 0 ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500' }}">
                                                        <div class="w-2 h-2 rounded-full {{ $item->scholarshipRequirements->count() > 0 ? 'bg-blue-500' : 'bg-slate-400' }}"></div>
                                                        {{ $item->scholarshipRequirements->count() }} Beasiswa
                                                    </span>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <div class="relative flex justify-center" x-data="{ open: false }">
                                                        ...
                                                        <div x-show="open"
                                                            ...
                                                            <form action="{{ route('requirements.destroy', $item->id) }}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" onclick="return confirm('Yakin ingin menghapus persyaratan ini?')"
                                            ...
                                            <tr>
                                                <td colspan="4" class="px-6 py-16 text-center">
                                                    <div class="flex flex-col items-center justify-center">
                                                        ...
                                                        <h3 class="text-lg font-semibold text-slate-700">Tidak ada persyaratan ditemukan</h3>
                                                        <p class="text-sm text-slate-500 mt-1">Belum ada persyaratan yang ditambahkan.</p>
                                                        <a href="{{ route('requirements.create') }}" class="mt-4 inline-block text-sm text-blue-600 hover:text-blue-800 font-medium hover:underline">+ Tambah Persyaratan</a>
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforelse
