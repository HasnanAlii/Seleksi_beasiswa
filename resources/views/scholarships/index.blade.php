                    ...
                    <div class="p-6 lg:p-10">
                        ...
                        <div class="relative z-[250] flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-6">
                            <div>
                                ...
                                <p class="text-sm text-slate-500 mt-1">Kelola data terkait beasiswa pada sistem beasiswa.</p>
                            </div>
                            <div class="flex gap-3">
                                <a href="{{ route('scholarship-types.index') }}"
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5l5.414 5.414A2 2 0 0118 9.828V19a2 2 0 01-2 2H6a2 2 0 01-2-2V5a2 2 0 012-2z" />
                                    </svg>
                                    Jenis Beasiswa
                                </a>
                        ...
                        <div class="mb-8 rounded-2xl border border-slate-200 bg-slate-50/70 p-4 relative z-[100]">
                            <form id="filter-form" method="GET" action="{{ route('scholarships.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6 items-end relative z-[100]">
                                ...
                                <div class="flex flex-col xl:col-span-3">
                                    ...
                                    <div class="relative group">
                                        <input type="text" id="filter_search" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Cari nama atau jenis beasiswa..." class="w-full rounded-xl border border-slate-200 bg-white px-4 py-[7px] pr-10 text-[13px] font-bold focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 shadow-sm min-h-[38px] text-slate-600">
                                ...
                                <table class="min-w-full divide-y divide-slate-100">
                                    <thead class="bg-slate-50/80">
                                        <tr>
                                            ...
                                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Beasiswa</th>
                                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Jenis</th>
                                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Kuota</th>
                                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Periode Berlaku</th>
                                    ...
                                    <tbody class="divide-y divide-slate-100 bg-white">
                                        @forelse($data as $key => $item)
                                            <tr class="group hover:bg-blue-50/40 transition-colors duration-200">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-bold text-blue-600">{{ $item->scholarship_name }}</td>
                                                <td class="px-6 py-4 text-left text-sm text-slate-500 max-w-md truncate">{{ $item->scholarship_type }}</td>
                                                <td class="px-6 py-4 text-left text-sm text-slate-500 max-w-md truncate">{{ $item->quota }} Orang</td>
                                                <td class="px-6 py-4 text-left text-sm text-slate-500 max-w-md truncate">{{ $item->validity_period ? $item->validity_period->format('d/m/Y') : '-' }}</td>
                                            ...
                                            <tr>
                                                <td colspan="6" class="px-6 py-16 text-center">
                                                    <div class="flex flex-col items-center justify-center">
                                                        ...
                                                        <p class="text-sm text-slate-500 max-w-xs mx-auto mt-1">Silakan tambahkan data baru.</p>
                                                        <a href="{{ route('scholarships.create') }}" class="mt-4 text-sm text-blue-600 hover:text-blue-800 font-medium hover:underline">
                                                            + Tambah Data
                                                        </a>
                                                    </div>
                                                </td>
