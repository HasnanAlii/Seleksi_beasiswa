<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <h2 class="font-extrabold text-2xl text-gray-800 leading-tight tracking-tight">Fungsi Keanggotaan</h2>
            <nav class="flex text-sm font-medium text-gray-500">
                <a href="{{ route('dashboard') }}" class="hover:text-blue-600 cursor-pointer transition">Dashboard</a>
                <span class="mx-2">/</span>
                <span class="text-blue-600">Fungsi Keanggotaan</span>
            </nav>
        </div>
    </x-slot>
                ...
                <div class="bg-white shadow-xl shadow-slate-200/60 rounded-3xl border border-slate-100">
                    <div class="p-6 lg:p-10">
                        ...
                        <div
                            ...
                            <div>
                                <h3 class="text-xl font-bold text-slate-800">Daftar Fungsi Keanggotaan</h3>
                                <p class="text-sm text-slate-500 mt-1">Kelola nilai min, mid, dan max setiap label
                                    kriteria fuzzy.</p>
                            </div>
                        ...
                        <div class="mb-8 rounded-2xl border border-slate-200 bg-slate-50/70 p-4 relative z-10">
                            <form id="filter-form" method="GET" action="{{ route('fuzzy-memberships.index') }}"
                                ...
                                <div class="flex flex-col xl:col-span-4">
                                    <label for="filter_criteria"
                                        class="mb-1.5 block text-[10px] font-black uppercase tracking-[0.1em] text-slate-400">Filter
                                        Kriteria</label>
                                    <x-searchable-dropdown name="criteria_id" id="filter_criteria"
                                        placeholder="Semua Kriteria" :options="$criteriaList
                                            ->map(fn($c) => ['id' => $c->id, 'name' => $c->criteria_name])
                                            ->prepend(['id' => '', 'name' => 'Semua Kriteria'])" :value="$filters['criteria_id'] ?? ''"
                                            ...
                                            <tr class="group hover:bg-blue-50/40 transition-colors duration-200">
                                                ...
                                                <td class="px-6 py-4 text-left">
                                                    <div class="font-bold text-blue-600">
                                                        {{ $item->criteria->criteria_name }}</div>
                                                </td>
                                                <td class="px-6 py-4 text-left">
                                                    <div class="text-sm font-semibold text-slate-600">
                                                        {{ $item->scholarship->scholarship_name }}</div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <span
                                                        class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-black bg-rose-50 text-rose-600 border border-rose-100/50">
                                                        {{ number_format($item->min_value, 2) }}
                                                    </span>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <span
                                                        class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-black bg-blue-50 text-blue-600 border border-blue-100/50">
                                                        {{ number_format($item->mid_value, 2) }}
                                                    </span>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <span
                                                        class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-black bg-emerald-50 text-emerald-600 border border-emerald-100/50">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <div class="relative flex justify-center" x-data="{ open: false, top: 0, left: 0, toggle(el) { this.open = !this.open; if (this.open) { const r = el.getBoundingClientRect();
                                                        ...
                                                        <div x-show="open" @click.outside="open=false"
                                                            ...
                                                            style="display:none;">
                                                            <a href="{{ route('fuzzy-memberships.show', $item->id) }}"
                                                            ...
                                                            <a href="{{ route('fuzzy-memberships.edit', $item->id) }}"
                                                            ...
                                                            <form
                                                                action="{{ route('fuzzy-memberships.destroy', $item->id) }}"
                                                                method="POST"
                                                                data-confirm-message="Yakin ingin menghapus keanggotaan ini? Tindakan ini tidak dapat dibatalkan.">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit"
                                                ...
                                                <td colspan="7" class="px-6 py-16 text-center">
                                                    <div class="flex flex-col items-center justify-center">
                                                        ...
                                                        <h3 class="text-lg font-semibold text-slate-700">Tidak ada
                                                            keanggotaan ditemukan</h3>
                                                        <p class="text-sm text-slate-500 mt-1">Belum ada fungsi
                                                            keanggotaan yang ditambahkan.</p>
                                                        <a href="{{ route('fuzzy-memberships.create') }}"
                                                            class="mt-4 inline-block text-sm text-blue-600 hover:text-blue-800 font-medium hover:underline">+
                                                            Tambah Keanggotaan</a>
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforelse
